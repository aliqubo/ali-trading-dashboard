...
- Multi-symbol batching in `_resolve_symbols()` — the fourth gap named in §7.7 "Known gaps (Phase 7)" — is also closed: `test_symbol_resolution_maps_multiple_symbols_independently` (`backend/tests/test_trading_api.py`) seeds two distinct symbols for the same user, exercises `GET /trading/orders` (the shared `_resolve_symbols()` path used by all four list endpoints), and asserts each order's resolved `symbol.id`/`ticker` matches its own `symbol_id` — with an explicit guard (`resolved_a["id"] != resolved_b["id"]`) against every row collapsing onto the first or last symbol resolved in the batch. Included in the 54/54 full backend suite result below.
